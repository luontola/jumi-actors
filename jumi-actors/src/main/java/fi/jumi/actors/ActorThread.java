...
public interface ActorThread {
     ...
     * <p/>
     * After nobody is holding the actor's {@link ActorRef}, that actor will be
     * garbage collected. It is OK to create lots of short-lived actors.
     */
    <T> ActorRef<T> bindActor(Class<T> type, T rawActor);

    /**
     * Stops <em>all</em> actors which are bound to this {@code ActorThread}
     * after all previously sent messages to them have been processed. It is
     * not possible to stop just one actor from an {@code ActorThread}, though
     * due to garbage collection that is usually not needed.
     * <p/>
     * An alternative way to stop actors is to call {@link Thread#interrupt()}
     * on the thread where the actor is running, which will stop that
     * {@code ActorThread} immediately.
     */
    void stop();
}
